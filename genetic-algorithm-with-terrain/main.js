var numBots = 30; //number of bots to spawn
...
var ground1 = Bodies.rectangle(400, 600, 800, 100, {isStatic:true, friction:0.01});

//console.log(ground1.vertices);

var ground = generateGround();

//console.log(ground.vertices)
...
Events.on(runner, "afterTick", function(){
	...
	if(tickNum % generationTicks == 0){
		curGeneration++;
		document.getElementById("generation").innerHTML = "Generation: " + curGeneration;
		...
		for(var i = 0; i < botslength/2; i++){ //choose bots to reproduce; the better the bot, the more likely it will reproduce
			//if(Math.random() > sigmoid(i)){
				...
				var child_model = bots[i].makeChild(0.01);
				var childBot = new Bot(botSize);
				childBot.model = child_model;
